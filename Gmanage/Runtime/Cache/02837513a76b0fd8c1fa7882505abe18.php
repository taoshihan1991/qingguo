	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			<?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
				...
				<td><?php echo ($v["_name"]); echo ($v["cname"]); ?></td>
				<td><?php echo ($v["title"]); ?></td>
				<td><?php echo ($v["sort"]); ?></td>
				<td><?php if($v['display']): ?>显示<?php else: ?>隐藏<?php endif; ?></td>
				<td>
					<a href="<?php echo U('category/add',array('cid'=>$v['cid']));?>" class="btn  btn-primary btn-xs">添加子类</a>&nbsp;&nbsp;
					<a href="<?php echo U('category/edit',array('cid'=>$v['cid']));?>" class="btn  btn-primary btn-xs">编辑</a>&nbsp;&nbsp;
					<a href="<?php echo U('category/del',array('cid'=>$v['cid']));?>" class="btn  btn-danger btn-xs">删除</a>
				</td>
